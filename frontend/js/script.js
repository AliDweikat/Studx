...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const facultyGrid = document.querySelector('#facultiesGrid') || document.querySelector('.grid');
    if (facultyGrid) {
...
async function fetchMaterials() {
    ...
    try {
        ...
        if (user) {
            try {
                const voteStatusRes = await fetch(`${API_BASE_URL}/materials/vote-status/${user.id}`);
                if (voteStatusRes.ok) {
        ...
        materials.forEach(material => {
            ...
            const materialVoteStatus = voteStatus[material.id] || { hasLiked: false, hasDisliked: false };
                    ...
                    <div class="card-content">
                        <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 10px;">
                            ...
                            <div class="vote-container">
                                <button id="upvote-btn-${material.id}" class="vote-btn upvote" onclick="voteMaterial(${material.id}, 'upvote', event)" title="Upvote" style="${upvoteStyle}">
                                    👍 <span id="upvotes-${material.id}">${material.upvotes}</span>
                                </button>
                                <button id="downvote-btn-${material.id}" class="vote-btn downvote" onclick="voteMaterial(${material.id}, 'downvote', event)" title="Downvote" style="${downvoteStyle}">
                        ...
                        <div class="card" style="cursor: pointer;" onclick="trackCourseView(${course.id}); window.location.href='${getPagesPath('materials.html')}?courseId=${course.id}'">
                            <div class="card-content">
                                ...
                                <p style="color: var(--accent-main); font-weight: bold; font-size: 1.1rem;">
                                    ${view.viewCount} ${view.viewCount === 1 ? (t.view || 'view') : (t.views || 'views')}
                                </p>
                                <div class="action-row">
                                    <a href="${getPagesPath('materials.html')}?courseId=${course.id}" class="download-link" onclick="event.stopPropagation(); trackCourseView(${course.id});">${t.view_materials || "View Materials"} →</a>
                                </div>
...
async function loadUserProfile() {
    ...
    if (!user) {
        if (notLoggedIn) {
            ...
            notLoggedIn.textContent = t.login_required || 'Please log in to view your profile.';
        }
        if (profileContent) {
            profileContent.innerHTML = `<p style="text-align:center;"><a href="login.html" style="color: var(--accent-main);">${t.login_link || 'Log In'}</a></p>`;
        }
        return;
    }
    ...
    try {
        ...
        if (userData.coursesRecentlyViewed && userData.coursesRecentlyViewed.length > 0) {
        ...
        if (profileContent) {
            profileContent.innerHTML = `<p style="text-align:center; color: red;">${t.error_fetch || 'Error loading profile'}</p>`;
        }
    }
}